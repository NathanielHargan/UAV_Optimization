...
class MyTestCase(unittest.TestCase):
    ...
    def test_cantilever_rectangle(self):
        # Relevant
        ...
        for i in np.arange(0, 1.1, 0.1):
            logging.debug(
                "Shape z disp " + str(round(i, 2)) + ": " + str(beam_system.beams[0].return_shape_functions(i)[0][0]))

        for i in np.arange(0, 1.1, 0.1):
            logging.debug(
                "Shape y angle " + str(round(i, 2)) + ": " + str(beam_system.beams[0].return_shape_functions(i)[0][1]))

    def test_center_multidim(self):
        ...
        total_fea_disp = math.sqrt(
            beam_system.x_displacements[1] ** 2 + beam_system.y_displacements[1] ** 2 + beam_system.z_displacements[
                1] ** 2)
        total_fea_reaction = math.sqrt(
            beam_system.x_forces[0] ** 2 + beam_system.y_forces[0] ** 2 + beam_system.z_forces[0] ** 2)

        print(total_fea_disp, total_fea_reaction)
    ...
    def test_incline_boundary_conditions(self):
    ...
    def test_reversed_cantilever_rectangle(self):
        # Relevant
        L = 500
        I = (6 * 20 ** 3) / 12
        E = 71700
        ...
        print("stress: ", beam_system.beams[0].bending_stresses_0)
    ...
    def test_cantilever_annulus(self):
        ...
        print("stress: ", beam_system.beams[0].bending_stresses_0)
        print("expected stress: ",
              m / beam_system.beams[0].beam_type.cross_section_properties['second moment of area y'])
    ...
    def test_timoshenko_simply_supp_point(self):
        #
        ...
        for beam in beam_system.beams:
            print("Max Stress 0", beam.bending_stresses_0)
            print("Max Stress 1", beam.bending_stresses_1)

    def test_timoshenko_simply_supp(self):
        beam_system = BeamSystem("timoshenko_simply_supp")
        arm_beam = BeamType("rectangle", [25, 25], "Aluminum7075-T6", "arm_beam")
    ...
    def test_timoshenko_simply_supp_point_shear(self):
        beam_system = BeamSystem("timoshenko_simply_supp")
        arm_beam = BeamType("rectangle", [25, 25], "Aluminum7075-T6", "arm_beam")
        node_count = 21
        L = 50
    ...
    def test_timoshenko_stress_circle(self):
        #
        beam_system = BeamSystem("timoshenko_simply_supp")
    ...
    def test_timoshenko_stress(self):
        #
        beam_system = BeamSystem("timoshenko_simply_supp")
        arm_beam = BeamType("rectangle", [25, 25], "Aluminum7075-T6", "arm_beam")
        node_count = 21
    ...
    def test_transient_displacement(self):
        timesteps = 100
        node_count = 3
        L = 100
        ...
        print("U_static = ")
        print(beam_system.z_displacements_ts[node_count - 1])
