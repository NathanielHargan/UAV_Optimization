...
class Optimizer:
    ...
    def independent_properties_calc(self):
        ...
        self.wingspan = (self.parameters["propeller_spacing"] + self.parameters["propeller_radius"]) / math.sin(
            math.pi / self.parameters["blade_num"])

        self.mass_inital = self.mass(self.design_variables_initial_guess)
        self.moment_inital = self.moment(self.design_variables_initial_guess)
    ...
    def moment(self, design_variables):
        ...
        drone_geometry = DroneGeometry("drone",
                                       ...
                                       strut_beam_properties,
                                       hub_beam_properties,
                                       self.simulation_settings["hub_sections_moment"],
                                       [self.mass_battery_1, self.mass_battery_2])
        ...
        Mass_Model = BeamMass(d1_fea.beam_system)
    ...
    def constraint_calculations(self, design_variables, active_constraints, detailed_output=False, detailed_output_print=False):
        ...
        if detailed_output:
            ...
            if detailed_output_print:
                print("=== Optimizer Results ===")
                print(f"Wingspan: {self.wingspan} mm")
    ...
    def run_opt(self):
        ...
        boundaries = np.array(list(self.boundaries.values()))

        fdv_list = list(self.fixed_design_variables.values())

        x_short_index = []
        
        del_indexes = []
        for i in range(len(fdv_list)):
            if fdv_list[i] is False:
                x_short_index.append(i)
            else:
                # delete if it is constant
                del_indexes.append(i)

        x_init = np.delete(x_init, del_indexes)
        boundaries = np.delete(boundaries, del_indexes, axis=0)

        def objective_short(x_short):
            d_v = self.reform_dvs(x_short)
            return self.objective(d_v) * self.simulation_settings["obj_multiplier"]

        def constraint_calculations_opt(x_short):
            d_v = self.reform_dvs(x_short)
        ...
        results = scipy.optimize.minimize(objective_short,
                                          x_init,
                                          constraints=consts,
                                          bounds=boundaries,
                                          method='SLSQP',
